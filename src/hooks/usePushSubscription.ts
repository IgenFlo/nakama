...
function swReady(timeoutMs = 8000): Promise<ServiceWorkerRegistration> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("Service worker non disponible — ferme et rouvre l'app")), timeoutMs),
    ),
  ]);
}
...
export function usePushSubscription(): PushState {
  ...
  const subscribe = useCallback(async () => {
    ...
    if (perm !== "granted") {
      throw new Error("Permission refusée — autorise les notifications dans les réglages");
    }
    ...
    if (!publicKey) throw new Error("Clé VAPID manquante — contacte l'administrateur");
